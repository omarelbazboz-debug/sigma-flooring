        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body p-4">
                    <div class="row">
                        <div class="col-lg-12">
                            <form method="POST" action="{{ url('admin/dates/'.$date->id) }}" enctype="multipart/form-data" data-toggle="validator">
                                @csrf
                                @method('PATCH')
                                <div class="row">
                                    <div class="col-md-4 mb-3">
                                        <label class="">{{trans('home.title_en')}}</label>
                                        <input class="form-control" name="title_en" type="text" placeholder="{{trans('home.title_en')}}" value="{{$date->title_en}}" required>
                                    </div>

                                    <div class="col-md-4 mb-3">
                                        <label class="">{{trans('home.title_ar')}}</label>
                                        <input class="form-control" name="title_ar" type="text" placeholder="{{trans('home.title_ar')}}" value="{{$date->title_ar}}" required>
                                    </div>
                                    <div class="col-md-2 mb-3">
                                        <label class="">{{ trans('home.number') }}</label>
                                        <input class="form-control" name="number" type="number"
                                            placeholder="{{ trans('home.number') }}" value="{{ $date->number }}">
                                    </div>
                                    <div class="col-md-2 mb-3">
                                        <label class="">{{ trans('home.order') }}</label>
                                        <input class="form-control" name="order" type="number"
                                            placeholder="{{ trans('home.order') }}" value="{{ $date->order }}">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label class="">{{ trans('home.text_en') }}</label>
                                        <textarea class="form-control ckeditor-classic" name="text_en" placeholder="{{ trans('home.text_en') }}">{!! $date->text_en !!}</textarea>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label class="">{{ trans('home.text_ar') }}</label>
                                        <textarea class="form-control ckeditor-classic" name="text_ar" placeholder="{{ trans('home.text_ar') }}">{!! $date->text_ar !!}</textarea>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="">{{trans('home.open')}}</label>
                                        <input class="form-control" name="am" type="text" value="{{$date->am}}" placeholder="{{trans('home.open')}}">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="">{{trans('home.close')}}</label>
                                        <input class="form-control" name="pm" type="text" value="{{$date->pm}}" placeholder="{{trans('home.close')}}">
                                    </div>
                                ...
                                <div class="form-group">
                                    <div class="form-check d-flex">
                                        <input type="checkbox" value="1" name="status" id="switch" switch="success" {{($date->status == 1)? 'checked':''}} />
