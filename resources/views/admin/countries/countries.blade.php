...
@section('content')
    <div class="container-fluid">
                ...
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    ...
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">{{trans('home.countries')}}</li>
                        </ol>
                    </div>
        ...
        <a href="{{url('admin/countries/create')}}"><button class="btn ripple btn-primary col-md-2"><i class="fas fa-plus-circle"></i> {{trans('home.add')}}</button></a>
        <!-- End Page Header -->
                        ...
                        <tbody>
                            @foreach($countries as $country)
                                <tr id="{{$country->id}}">
                                    <td> 
                                        @if ($country->id!=1)
                                            <input type="checkbox" name="checkbox"  class="tableChecked" value="{{$country->id}}" />                                         
                                        @endif
                                    </td>
                                    ...
                                    <td> 
                                        <input class="btn_active" data-id="{{$country->id}}" type="checkbox" id="switch-{{$country->id}}" switch="success" {{$country->status == 1?'checked':''}} />
                                        <label for="switch-{{$country->id}}" data-on-label="{{trans('home.yes')}}" data-off-label="{{trans('home.no')}}"></label>
                                    </td>
                                    <td> 
                                        <a type="button" class="btn btn-info waves-effect waves-light " 
                                            href="{{ route('countries.edit',$country->id) }}" >{{__('home.edit')}}</a>
                                    </td>
                                    <td> 
                                        @if ($country->id!=1)
                                            <a type="button" class="btn btn-danger waves-effect waves-light btn_delete" 
                                                data-id="{{$country->id}}">{{__('home.delete')}}</a>
                                        @else
                                            <a>-</a>
                                        @endif
                                    </td>
